...
app.post([`${apiBasePath}/token`], (req, res) => {
    ...
    if (!api.isAuthorized()) {
        log.debug('FeatureMeet get token - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('FeatureMeet get token - Authorized', {
        header: req.headers,
        body: req.body,
        token: token,
    });
...
app.get([`${apiBasePath}/meetings`], (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('FeatureMeetTalk get meetings - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    // log.debug the output if all done
    log.debug('FeatureMeet get meetings - Authorized', {
        header: req.headers,
        body: req.body,
        meetings: meetings,
    });
...
app.post([`${apiBasePath}/meeting`], (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('FeatureMeet get meeting - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    log.debug('FeatureMeet get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
...
app.post([`${apiBasePath}/join`], (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('FeatureMeet get join - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    log.debug('FeatureMeet get join - Authorized', {
        header: req.headers,
        body: req.body,
        join: joinURL,
    });
